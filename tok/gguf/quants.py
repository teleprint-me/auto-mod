...
from .constants import GGUF_QUANT_SIZES, GGUFQuantizationType
from .lazy import LazyNumpyTensor

import numpy as np


def quant_shape_to_byte_shape(shape: Sequence[int], quant_type: GGUFQuantizationType):
...
def quant_shape_from_byte_shape(shape: Sequence[int], quant_type: GGUFQuantizationType):
    block_size, type_size = GGUF_QUANT_SIZES[quant_type]
...
def __quantize_bf16_array(n: np.ndarray) -> np.ndarray:
    return __apply_over_grouped_rows(
        __compute_fp32_to_bf16, arr=n, otype=np.int16, oshape=n.shape
    )


__quantize_bf16_lazy = LazyNumpyTensor._wrap_fn(
    __quantize_bf16_array, meta_noop=np.int16
)
...
__q8_block_size, __q8_type_size = GGUF_QUANT_SIZES[GGUFQuantizationType.Q8_0]
